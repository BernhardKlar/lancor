lcor.ci = function(x, y = NULL, conf.level = 0.95, type = c("rank", "linear"),
  ...
  if (type == "rank") { 
    ...
    Sigma =  n * cov( boot(xx, statistic = function(xx, id) lcor.comp( xx[id, ], type="rank"), R = R)$t[,1:2] )
    #case |rho1| != |rho2|
    ...
    F = function(x, Sigma, p) {  
      ...
      0.5 * ( psn(x,0,sigma1,alpha1) + psn(x,0,sigma2,alpha2) ) - p
    }
  ...
  if (type == "linear") { 
    ...
    if (method == "plugin" | method == "boot") { 
      ...
      if (method == "boot") { #bootstrap estimate of covariance matrix 
        Sigma = n * cov( boot(xx, statistic = function(xx, id) lcor.comp(xx[id, ], type="linear"), R = R)$t[,1:2] )    
      }
      #case |rho1| != |rho2|
      if (abs(rho1) >= abs(rho2)) {
        se = sqrt(Sigma[1,1])
